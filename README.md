...
局域网自部署的图表分享应用：上传 CSV 生成 Plotly 图表，并提供**稳定不变**的页面 / PNG / SVG 链接，可拷贝插入笔记中长期引用（记录删除前链接始终有效）。
...
- 每张图分配唯一不可猜测的短 ID，提供三种稳定地址：
  - 交互页：`/chart/<id>` —— Plotly 悬停看数值、缩放
  - 静态图：`/chart/<id>.png`、`/chart/<id>.svg` —— 可用 `![](url)` 嵌入笔记
- 首页可进入编辑模式，多选并删除记录（彻底删除：DB、CSV 与缓存图一并清除，对应链接随之失效）
