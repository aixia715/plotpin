...
- 数据规模:本版假设单 CSV 数据点在几千量级(用户自控),交互图内嵌全部点;不做降采样。
- 首页缩略图:每条记录仅当 `.png` 缓存文件**已存在**时用其作缩略图,否则占位;首页**不触发渲染**。
- 对数坐标:每轴独立开关 `x_log`/`y_log`(默认线性),与工程计数法显示正交。轴设为对数但其数据含 `≤0` 一律拒绝(不静默丢点)。对数轴刻度用 10 的整数幂位置,文字仍按对应轴 `*_eng` 格式化。
...
- `app/__init__.py` — 包标记。
- `app/eng_notation.py` — 工程计数法解析与数字/刻度格式化:`parse_eng`、`format_eng`、`format_plain`、`nice_ticks`、`log_ticks`、`EngParseError`、`PREFIXES`。
- `app/ids.py` — `new_id`。
- `app/parsing.py` — CSV 编码识别、读取、校验:`decode_bytes`、`read_csv_bytes`、`check_log_positivity`、`ParsedCSV`、`CSVParseError`。
...
- Produces:
  ...
  - `nice_ticks(lo: float, hi: float, target: int = 6) -> list[float]` — 生成线性轴的"漂亮"刻度位置。
  - `log_ticks(lo: float, hi: float) -> list[float]` — 生成对数轴的十进制(10 的整数幂)刻度位置;要求 `lo`、`hi` 均 `>0`。
...
Run: `pytest tests/test_eng_notation.py -k "log_ticks" -v`
Expected: FAIL(`NameError`)

- [ ] **Step 14: 实现 log_ticks**
...
git commit -m "feat: engineering notation parse/format and tick helpers"
```
...
- Produces:
  ...
  - `check_log_positivity(parsed: ParsedCSV, x_log: bool, y_log: bool) -> None` — 若 `x_log` 且 X 含 `≤0`,或 `y_log` 且任一 Y 含 `≤0`,抛 `CSVParseError`(中文提示);否则无返回。
...
- [ ] **Step 5: 写失败测试(check_log_positivity)**

追加到 `tests/test_parsing.py`:
```python
def _parsed():
    return ParsedCSV(
...
Run: `pytest tests/test_parsing.py -k "log" -v`
Expected: FAIL(`ImportError`/未定义)

- [ ] **Step 7: 实现 check_log_positivity**
...
git commit -m "feat: CSV decoding, parsing, strict validation and log positivity check"
```
...
- Consumes: 无。
- Produces:
  - `@dataclass Chart` 字段:`id: str`、`title: str`、`x_title: str`、`y_title: str`、`x_eng: bool`、`y_eng: bool`、`x_log: bool`、`y_log: bool`、`created_at: str`。
  - `class Storage`:
    - `__init__(self, data_dir: str | Path)` — 建好 `csv/`、`cache/` 目录与库表。
    - `exists(self, chart_id: str) -> bool`。
    - `save_chart(self, chart_id, title, x_title, y_title, x_eng, y_eng, x_log, y_log, csv_bytes: bytes) -> Chart`。
...
**Interfaces:**
- Consumes: `app.parsing.ParsedCSV`;`app.eng_notation.format_eng`、`format_plain`、`nice_ticks`、`log_ticks`。
- Produces:
  - `render_static(parsed, title, x_title, y_title, x_eng, y_eng, x_log, y_log, fmt) -> bytes` — `fmt` 取 `"png"` 或 `"svg"`;`x_log`/`y_log` 为 `bool`,对数轴用 `set_xscale/set_yscale("log")`。返回图片字节。
  - `build_plotly_spec(parsed, title, x_title, y_title, x_eng, y_eng, x_log, y_log) -> dict` — 返回含 `"data"` 与 `"layout"` 两键的 dict;对数轴时 `layout[axis]["type"]="log"` 且 tickvals 用 `log_ticks`(线性轴用 `nice_ticks`)。
...
注意:Plotly 对数轴的 `tickvals` 用实际数据空间的值(即 10 的整数幂),Plotly 会按对数定位;`ticktext` 仍走 `eng_notation` 格式化,保证与线性轴风格一致。
...
> **HTTP 下的复制:** 部署是局域网纯 HTTP(非 localhost),`navigator.clipboard` 在非安全上下文不可用。`chart.html` 的链接复制**必须**带 `document.execCommand('copy')` 兜底,不能只用 Clipboard API。实现见下方 `chart.html` 内的 `copyText`。

**Interfaces:**
- Consumes: `app.storage.Storage`、`app.parsing.read_csv_bytes`/`check_log_positivity`/`CSVParseError`、`app.rendering.render_static`/`build_plotly_spec`、`app.ids.new_id`、`app.eng_notation`。
- Produces(路由):
  - `GET /` — 上传区 + 图表清单(倒序)。
  - `POST /charts` — 表单 `file`(UploadFile)、`title`、`x_title`、`y_title`、`x_eng`/`y_eng`/`x_log`/`y_log`(均可选 checkbox);成功 303 跳转 `/chart/{id}`,失败 400 重渲染首页带中文错误。
...
<body>
  ...
  <form id="upload-form" action="/charts" method="post" enctype="multipart/form-data">
    ...
    <div id="config" hidden>
      ...
      <fieldset>
        <legend>X 轴</legend>
        <label><input type="checkbox" name="x_eng" checked> 工程计数法显示</label>
        <label><input type="checkbox" name="x_log"> 对数坐标</label>
      </fieldset>
      <fieldset>
        <legend>Y 轴</legend>
        <label><input type="checkbox" name="y_eng" checked> 工程计数法显示</label>
        <label><input type="checkbox" name="y_log"> 对数坐标</label>
      </fieldset>
  ...
  <ul class="chart-list">
    {% for it in items %}
      <li>
        {% if it.thumb %}
          <img class="thumb" src="{{ it.thumb }}" alt="" width="64" height="34">
        {% else %}
          <span class="thumb placeholder"></span>
        {% endif %}
        <a href="/chart/{{ it.chart.id }}">{{ it.chart.title }}</a>
        <small>{{ it.chart.created_at }}</small>
      </li>
    {% else %}
      <li>暂无图表</li>
    {% endfor %}
...
<body>
  ...
  <div class="links">
    <div class="link"><span class="tag">交互页</span><code class="url" id="link-page"></code><button class="copy" data-target="link-page">复制</button></div>
    <div class="link"><span class="tag">PNG</span><code class="url" id="link-png"></code><button class="copy" data-target="link-png">复制</button></div>
    <div class="link"><span class="tag">SVG</span><code class="url" id="link-svg"></code><button class="copy" data-target="link-svg">复制</button></div>
  </div>
  ...
  <script>
    ...
    // HTTP(非安全上下文)下 navigator.clipboard 不可用,用 execCommand 兜底
...
#config fieldset { border: 1px solid #ddd; }
#config legend { font-size: .8rem; }
.chart-list { line-height: 1.8; list-style: none; padding: 0; }
.chart-list li { display: flex; align-items: center; gap: .6rem; }
.thumb { width: 64px; height: 34px; object-fit: contain; background: #f3f3f3; }
.thumb.placeholder { display: inline-block; }
.links .link { display: flex; align-items: center; gap: .6rem; margin: .4rem 0; }
.links .tag { font-size: .75rem; min-width: 4rem; }
.links code.url { background: #f3f3f3; padding: .1rem .3rem; flex: 1; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; cursor: pointer; }
.links .copy { cursor: pointer; }
```
...
- 配置三标题 + 每轴(工程计数法 + 对数)四个勾选 → Task 7(表单/模板)、Task 5(存储字段 x_eng/y_eng/x_log/y_log)。
- 对数坐标(每轴可选)+ ≤0 严格拒绝 → Task 2(`log_ticks`)、Task 4(`check_log_positivity`)、Task 6(log scale)、Task 7(表单/校验/报错)。
...
**Type consistency:** `ParsedCSV`(x_label/x/y_labels/ys)、`Chart`(含 x_eng/y_eng/x_log/y_log)、`Storage` 方法名、`render_static`/`build_plotly_spec` 签名(均含 x_eng,y_eng,x_log,y_log)、`get_storage` 在 Task 1 占位并在 Task 7 落地——前后一致。`x_eng`/`y_eng`/`x_log`/`y_log` 全程 `bool`,存库转 `int`。`save_chart` 参数顺序 `(id,title,x_title,y_title,x_eng,y_eng,x_log,y_log,csv_bytes)` 与 Task 5/7 调用一致。
